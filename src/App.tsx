...
function App() {
  ...
  const showCard = () => {
    ...
    setTimeout(() => {
      firstCard?.setAttribute('src', `https://deckofcardsapi.com/static/img/${CPU1First}.png`);
    }, 500);

    setTimeout(() => {
      secondCard?.setAttribute('src', `https://deckofcardsapi.com/static/img/${CPU1Second}.png`);
    }, 1000);

    setTimeout(() => {
      thirdCard?.setAttribute('src', `https://deckofcardsapi.com/static/img/${CPU2First}.png`);
    },2500);

    setTimeout(() => {
      fourthCard?.setAttribute('src', `https://deckofcardsapi.com/static/img/${CPU2Second}.png`);
    }, 3000);
...
const refresh = () => {
  ...
  return (
    <div className="App" id='app'>
      ...
      {!isStarted && <div className='enemy-card-wrap'>
        <div className='side-enemy'>
          <img id="1" src='https://i.pinimg.com/236x/62/72/c7/6272c70b7708ca03f50284a9720c98a3.jpg' className='enemy-card-left'/>
          <img id="2" src="https://i.pinimg.com/236x/62/72/c7/6272c70b7708ca03f50284a9720c98a3.jpg" className='enemy-card-right'/>
        </div>
        <div className='side-enemy'>
          <img id="3" src='https://i.pinimg.com/236x/62/72/c7/6272c70b7708ca03f50284a9720c98a3.jpg' className='enemy-card-left'/>
          <img id="4" src="https://i.pinimg.com/236x/62/72/c7/6272c70b7708ca03f50284a9720c98a3.jpg" className='enemy-card-right'/>
        </div>
      </div>}
      <div className="card-wrap">
        {isStarted && <button className='button' onClick={callApi}>PLAY</button>}
        <div className="card-left" id="leftObject" onClick={changeLeft}></div>
        <div className="card-right right-content" id="rightObject" onClick={changeRight}></div>
      </div>
      <div>
        {showResult && <div className='neon'>{result}</div>}
        {showResult && <button className='button' onClick={refresh}>BACK TO TITLE</button>}
      </div>
      <div className="button-warp">
        {setGame && <button id='changeButton' className='enabled button left' disabled={disable} onClick={changeCards}>CHANGE</button>}
        {setGame && <button id='openButton' className='button right-content' disabled={!disable} onClick={open}>OPEN</button>}
      </div>
    </div>
  )
}
